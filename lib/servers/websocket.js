...
module.exports = function(server, websocketConfig, secret) {
    ...
    wildcardChannel.onCreate = function(nsp) {
    ...
    wildcardChannel.onClientConnect = function(client) {
        ...
        client.on('disconnect', function() {
            console.log('disconnected from', client.nsp.fullname());
            client.broadcast.emit('user:remove.custom', message.update('server', null, client.decoded_token.id));
        });
    };
}