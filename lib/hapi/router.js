...
module.exports = {
    createDefaultRoutes: function(server, root, options) {
        ...
        server.register([require('hapi-auth-jwt2'), require('h2o2'), require('hapi-heroku-helpers')], function (err) {
            if(err){
                console.log(err);
            }
            ...
            server.register(options.routes.map(function(config) {
                return {
                    ...
                    options: lazy.mixin(options.options, {root: root}, config.options)
                };
            }), function (err) {
                if(err){
                    console.log(err);
